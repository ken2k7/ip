...
This is an introductory Java software-engineering project. When presenting its
changes, keep any accompanying explanation brief and student-oriented: identify
the changed behavior, the relevant Java/OOP idea, and any obvious iP rubric risk
(such as missing tests, error handling, Javadoc, or a GUI requirement). A visual
diff is for review only; do not change source files or create commits unless the
user separately asks.

Use Java 25 for any optional project verification. Do not commit or push as part
of generating a diff page.
